...
Headplane_ is a feature-complete web UI for Headscale, allowing you to manage your nodes, networks, and ACLs with ease. It is open source and integrates directly with your Headscale instance.
...
.. code-block:: ini
  ...
  command=pnpm start -- --config %(ENV_HOME)s/opt/headplane/config.yaml
  autostart=true
  autorestart=true
  stopsignal=INT
  startsecs=5
...
.. code-block:: console
  ...
  [isabell@stardust ~]$ uberspace web backend set /admin --http --port 3000
  Set backend for /admin to port 3000; please make sure something is listening!
  You can always check the status of your backend using "uberspace web backend list".
  [isabell@stardust ~]$

.. note::
   By default, Headplane serves its UI at ``/admin``. Adjust the path if you change the admin prefix in your config and web backend.
...
Point your browser to ``https://<your-domain>/admin`` and log in with your admin credentials.
...
- https://github.com/tale/headplane/blob/main/docs/Bare-Metal.md
