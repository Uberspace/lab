...
If there is a new version available, update the ``hugo`` binary in ``~/bin`` (repeat Step 3). It might be a good idea to rebuild your site, too, but that's not strictly neccessary.
