...
.. sidebar:: About
  ...
  .. image:: _static/images/etherpad.png
      :align: center
...
First get the Etherpad Lite source code from Github_, watch out to replace the pseudo version number ``66.6.6`` here with the latest version number from the release feed_:

.. code-block:: console
 :emphasize-lines: 1

  [etherpad@stardust ~]$ git clone --branch release/66.6.6 https://github.com/ether/etherpad-lite ~/etherpad
  Cloning into '/home/etherpad/etherpad'...
...
.. code-block:: console
  ...
  [etherpad@stardust ~]$ mysql -e "CREATE DATABASE ${USER}_etherpad COLLATE utf8mb4_unicode_ci;"
  [etherpad@stardust ~]$
...
.. code-block:: console
  ...
  [etherpad@stardust ~]$ cd ~/etherpad
  [etherpad@stardust ~]$ git pull origin release/66.6.6
...
.. code-block:: console
  ...
  [etherpad@stardust ~]$ supervisorctl restart etherpad
  etherpad: stopped
  etherpad: started
  [etherpad@stardust ~]$
