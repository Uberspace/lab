...
Setup your URL for the front end:
...
In PostgreSQL you need a database with your username as name. Otherwise there is an error when you create the database for pleroma. (//This is maybe a bug in the pleroma installation or PostgreSQL. I dont know.)
...
::
 ...
 What is the password used to connect to your database? [autogenerated]  MySecretPassword
...
After this you will also have the PostgreSQL file ``config/setup_db.psql``. This script creates the user ``pleroma`` with your password ``MySecretPassword`` and the database ``pleroma`` you gave above with the user ``pleroma`` as owner. Additional it install the extensions ``citext``, ``pg_trgm`` and ``uuid-ossp`` to the database. 
Run the script with ``psql``:

.. Ich habe festgestellt, das beim Ausführen der Datei die Meldung kam: "Konnte die Datenbank <username> nicht finden." Wie ganz am Anfang angesprochen habe ich dafür eine Datenbank erstellt die den gleichen Namen hatte, womit ich psql ausführe. (Wenn du dem Guide in :lab:`PostgreSQL <guide_postgresql>` genau gefolgt bist, ist dieser identisch mit einem Uberspace Namen.)
...
.. code-block:: ini
 ...
 ; Also assumes mix is in /home/<username>/pleroma, this might differ on BSDs or niche Linux distros
